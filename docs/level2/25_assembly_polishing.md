...
??? success "Output"
    ...
    ```
    [M::mm_idx_gen::0.011*0.96] collected minimizers
    [M::mm_idx_gen::0.019*1.86] sorted minimizers
    [M::main::0.019*1.86] loaded/built the index for 1 target sequence(s)
    [M::mm_mapopt_update::0.020*1.82] mid_occ = 10
    [M::mm_idx_stat] kmer size: 15; skip: 10; is_hpc: 0; #seq: 1
    [M::mm_idx_stat::0.020*1.79] distinct minimizers: 47524 (99.64% are singletons); average occurrences: 1.004; average spacing: 5.333; total length: 254357
    [M::worker_pipeline::2.259*1.85] mapped 1766 sequences
    [M::main] Version: 2.31-r1302
    [M::main] CMD: /opt/conda/envs/bio/bin/minimap2 -t 4 -ax map-ont draft_genomes/draft_moderate.fna reads/Mbovis_87900.nanopore.fq.gz
    [M::main] Real time: 2.262 sec; CPU: 4.190 sec; Peak RSS: 0.190 GB
    ```
...
??? success "Output"
    ...
    ```
    [racon::Polisher::initialize] loaded target sequences 0.000823 s
    [racon::Polisher::initialize] loaded sequences 0.288706 s
    [racon::Polisher::initialize] loaded overlaps 0.056235 s
    [racon::Polisher::initialize] aligning overlaps [====================] 0.040482 s
    [racon::Polisher::initialize] transformed data into windows 0.041661 s
    [racon::Polisher::polish] generating consensus [====================] 11.793202 s
    [racon::Polisher::] total = 12.223761 s
    ```
...
It is possible to perform the `racon` process iteratively, remapping reads to the output and then running the polishing cycle again. However, there are costs associated with this approach both in terms of time invested and over-zealous correction to repeat regions. Whether or not improvement with multiple rounds will be seen in your data is uncertain, and ultimately it is your decision whether or not to perform this approach so although this can work, it is a judgement call as to whether or not it is necessary.
...
!!! terminal "code"
    ...
    minimap2 -t 4 -ax map-ont draft_moderate.racon_1.fna reads/Mbovis_87900.nanopore.fq.gz > draft_moderate.racon_1.sam

    # Perform correction, creating a new output file for the polished genome
    racon -t 4 reads/Mbovis_87900.nanopore.fq.gz draft_moderate.racon_1.sam draft_moderate.racon_1.fna > draft_moderate.racon_2.fna
    ```
...
!!! question "Exercise"
    ...
    ??? circle-check "Solution"
        ...
        !!! terminal "code"
            ...
            for assembly in draft_mild draft_severe;
            do
                minimap2 -t 4 -ax map-ont ${assembly}.racon_1.fna reads/Mbovis_87900.nanopore.fq.gz > ${assembly}.racon_1.sam
                racon -t 4 reads/Mbovis_87900.nanopore.fq.gz ${assembly}.racon_1.sam ${assembly}.racon_1.fna > ${assembly}.racon_2.fna
            done
            ```
...
!!! question "Exercise"
    ...
    Set a command to run `QUAST` again, this time checking the raw and polished genomes against the reference.
    ...
    ??? circle-check "Solution"
        ...
        !!! terminal "code"
            ...
            quast.py -r reference/Mbovis_87900.genome.fna --gene-finding -o quast/ \
                draft_genomes/draft_moderate.fna \
                draft_moderate.racon_1.fna \
                draft_moderate.racon_2.fna
                # ...plus your genomes
            ```
