...
Navigate to the `assembly_illumina/` folder and run a basic `SPAdes` assembly.

!!! terminal "code"

    ```bash
    cd level2/assembly_illumina/

    spades.py --isolate --threads 4 \
...
!!! info "Different modes for running `SPAdes`"
    ...
    1. isolate - assembly of a single genome (what we used here)
